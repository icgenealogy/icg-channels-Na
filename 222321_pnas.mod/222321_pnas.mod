TITLE pnas :Transient sodium current in the pyram soma
...
FUNCTION h_a(v(mV)) {
	TABLE FROM -150 TO 150 WITH 301
:	h_a = 0.32*exp((42-v-60+10)/18)
	h_a = 0.32*exp((42-v-60+5)/18)
}

FUNCTION h_b(v(mV)) {
	TABLE FROM -150 TO 150 WITH 301
:	h_b = 10/(1+exp((42-v-60+10)/5))
	h_b = 10/(1+exp((42-v-60+5)/5))
}
