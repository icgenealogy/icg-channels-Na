...
NEURON {
	...
	RANGE  gbar, ar2, pert, h, s, m, pert1, pert2
...
PROCEDURE trates(vm,a2) {  
	...
	a = trap1(vm,thi1,Rd,qd)
	b = trap1(-vm,-thi2,Rg,qg)
	htau =  1/(a+b)/qt
        if (htau<hmin) {htau=hmin}
	hinf = 1/(1+exp((vm-thinf+pert1)/qinf))
	c=alpv(vm)
        sinf = c+a2*(1-c)
...
FUNCTION trap0(v,th,a,q) {
	if (fabs(v-th) > 1e-6) {
	        trap0 = a * (v - th + pert) / (1 - exp(-(v - th + pert)/q))
	} else {
	        trap0 = a * q
 	}
}	

FUNCTION trap1(v,th,a,q) {
	if (fabs(v-th) > 1e-6) {
	        trap1 = a * (v - th + pert2) / (1 - exp(-(v - th + pert2)/q))
	} else {
	        trap1 = a * q
 	}
}